...
const MyAppointments = () => {
  ...
  return (
    <div>
      ...
      <div>
      {
        treatmentsData.slice(0, 2).map((item, index) => (
          <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b" key={index}>
            <div>
              <img className="w-32 bg-indigo-50" src={item.image} alt="" />
            </div>
            <div className="flex-1 text-sm text-zinc-600">
              <p className="text-neutral-800 font-semibold">{item.service}</p>
              <p>{item.about}</p>
              {/* <p className="text-zinc-700 font-medium mt-1">Address:</p> */}
              {/* <p className="text-xs">{item.address.line1}</p>
              <p className="text-xs">{item.address.line2}</p> */}
              <p className="text-xs"><span className="text-sm text-neutral-700 font-medium">Date & Time:</span> 25, November, 2024 | 8:30 AM</p>
              <p></p>
            </div>
            <div></div>
